...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Title 'Eureka Server'; java -jar backend/discovery-service/target/discovery-service-1.0.0.jar" -WindowStyle Normal
...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Title 'User Service (Auth)'; java -jar backend/user-service/target/user-service-1.0.0.jar" -WindowStyle Normal

Write-Host "3. Starting Product Catalog & Inventory Service [Port 8082]..." -ForegroundColor Green
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Title 'Product Service'; java -jar backend/product-service/target/product-service-1.0.0.jar" -WindowStyle Normal

Write-Host "4. Starting Order Checkout & Shopping Cart Service [Port 8083]..." -ForegroundColor Green
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Title 'Order Service'; java -jar backend/order-service/target/order-service-1.0.0.jar" -WindowStyle Normal
...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Title 'API Gateway'; java -jar backend/gateway-service/target/gateway-service-1.0.0.jar" -WindowStyle Normal
